...
def render(state,angle):
 ...
 for m in ms:
  v=np.asarray(m['positions']).reshape(-1,3)@rot.T;f=np.asarray(m['indices']).reshape(-1,3);norm=np.zeros_like(v);cr=np.cross(v[f[:,1]]-v[f[:,0]],v[f[:,2]]-v[f[:,0]])
  for k in range(3):np.add.at(norm,f[:,k],cr)
  norm/=np.maximum(np.linalg.norm(norm,axis=1)[:,None],1e-10);scr=np.stack([v[:,0]*385+W/2,H-62-v[:,1]*385,v[:,2]],axis=1);col=np.asarray(m['color']);lum=.48+.49*np.maximum(0,norm@light)
  for ids in f:
   ...
   yy,xx=np.mgrid[y0:y1+1,x0:x1+1];xx+=.5;yy+=.5;u=((by-cy)*(xx-cx)+(cx-bx)*(yy-cy))/den;vv=((cy-ay)*(xx-cx)+(ax-cx)*(yy-cy))/den;w=1-u-vv;dep=u*t[0,2]+vv*t[1,2]+w*t[2,2];buf=zbuf[y0:y1+1,x0:x1+1];mask=(u>=0)&(vv>=0)&(w>=0)&(dep<buf)
 ...
 im=Image.fromarray((rgb*255).astype('uint8'));d=ImageDraw.Draw(im);font=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',17);d.text((22,18),f'MODEL003 ADULT FEMALE CANDIDATE 001 | {angle} DEG',fill='#294359',font=font);p=OUT/f'adult-female-candidate-rest-{angle}.png';im.save(p);return im
