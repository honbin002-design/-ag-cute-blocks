...
def render(state,angle,base=False):
    ...
    im=Image.fromarray((rgb*255).astype('uint8'));d=ImageDraw.Draw(im);d.text((24,18),f'MODEL003 ADULT FEMALE CANDIDATE 003 | {state.upper()} | {angle} DEG | V0.4.96',fill='#294359',font=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',18));d.text((24,H-27),'INDEPENDENT CANDIDATE / ACTUAL EXPORTED GLB / NOT GAME INTEGRATED',fill='#496071',font=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',13));p=OUT/f'adult-female-candidate-003-{state}-{angle}{"-base" if base else ""}.png';im.save(p);print(p,flush=True);return im
if __name__=='__main__':
    ...
    sheet.save(OUT/'adult-female-candidate-003-eight-view.png')
