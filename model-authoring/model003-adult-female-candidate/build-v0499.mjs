...
function eachPosition(nodeName,fn){const {a,start,stride}=accessorForNode(nodeName);for(let i=0;i<a.count;i++){const p=start+i*stride,x=bin.readFloatLE(p),y=bin.readFloatLE(p+4),z=bin.readFloatLE(p+8),q=fn(x,y,z);if(q){bin.writeFloatLE(q[0],p);bin.writeFloatLE(q[1],p+4);bin.writeFloatLE(q[2],p+8)}}}
