...
function applyAsset(group,c,gltf){
  ...
  tuneRigForCustomization(root,c);root.updateMatrixWorld(true);let fitBox=new THREE.Box3().setFromObject(selectedVariant||root),fitSize=fitBox.getSize(new THREE.Vector3()),fit=Math.min(1,1.16/Math.max(.001,fitSize.x),.92/Math.max(.001,fitSize.z));if(fit<1){root.scale.multiplyScalar(fit);root.updateMatrixWorld(true)}const box=new THREE.Box3().setFromObject(selectedVariant||root),center=box.getCenter(new THREE.Vector3());root.position.x-=center.x;root.position.z-=center.z;root.position.y-=box.min.y;root.updateMatrixWorld(true);setAssetMaterialPolish(root,c);if(c.role==='special3')setSpecial3GoldMaterials(root);
