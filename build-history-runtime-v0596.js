// AG Cute Blocks V0.5.96 — safe build-only Undo / Redo.
...
const STACK_KEY='ag_cute_blocks_build_history_v0596';
...
function push(change){stacks.undo.push(change);if(stacks.undo.length>MAX_HISTORY)stacks.undo.shift();stacks.redo=[];saveStacks()}
...
function applyChange(change,direction){
  forceSave();const world=readWorld();if(!world)return false;
  const target=direction==='undo'?change.before:change.after;
  const other=direction==='undo'?change.after:change.before;
  let ok=false;
  if(target===null)ok=deleteEntity(world,other);else ok=replaceEntity(world,target);
  if(!ok)return false;world.savedAt=Date.now();writeWorld(world);return true;
}
...
globalThis.__AGCB_BUILD_HISTORY={version:VERSION,status:'SAFE_BUILD_ONLY',maxHistory:MAX_HISTORY,safeObjectTypes:[...SAFE_OBJECT_TYPES],undo,redo,get undoCount(){return stacks.undo.length},get redoCount(){return stacks.redo.length}};
