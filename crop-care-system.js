...
export const CROP_CARE_SCHEMA=2;
...
const naturalRain=weather=>weather==='rain'||weather==='thunderstorm';

export function createCropCare(saved={}){
  return {schema:CROP_CARE_SCHEMA,wateredDay:Number.isFinite(Number(saved.wateredDay))?Number(saved.wateredDay):-1,lastCareDay:Number.isFinite(Number(saved.lastCareDay))?Number(saved.lastCareDay):-1,sparkleUntil:Number(saved.sparkleUntil)||0};
}
...
export function isWateredToday(care,worldDay,weather='sunny'){return naturalRain(weather)||care.wateredDay===worldDay}
export function wetnessVisual(care,worldDay,weather='sunny'){
  if(naturalRain(weather))return 1;
  return care?.wateredDay===worldDay?1:0;
}
export function dailyCropGrowth(kind,care,worldDay,weather='sunny',seasonAllowed=true){
  if(!seasonAllowed)return 0;const r=CROP_CARE_RULES[kind]||{base:.085,waterBonus:.07,rainBonus:.085};
  if(naturalRain(weather))return r.base+r.rainBonus;if(care.wateredDay===worldDay)return r.base+r.waterBonus;return r.base;
}
export function cropCareLabel(care,worldDay,weather='sunny'){
  if(naturalRain(weather))return weather==='thunderstorm'?'⛈️ 雷雨已澆灌':'🌧️ 雨水已澆灌';if(care.wateredDay===worldDay)return '💧 今天澆過水';return '🪴 可澆水加速成長';
}
