...
function normalize(root,targetHeight=2.0){const box=new THREE.Box3().setFromObject(root),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3());root.position.sub(center);root.position.y+=size.y/2;root.scale.multiplyScalar(targetHeight/Math.max(size.y,.001));return root}
...
async function create(id){const spec=TEST_CHARACTERS[id];if(!spec)throw new Error(`Unknown test character: ${id}`);const model=await load(spec.model);const root=normalize(model.scene);root.name=`ag-${id}`;root.userData={...(root.userData||{}),agTestCharacter:id,label:spec.label};let clips=model.animations||[];if(spec.animations){const lib=await load(spec.animations);if(lib.animations?.length)clips=lib.animations}const mixer=new THREE.AnimationMixer(root),actions={};const map={idle:['idle'],walk:['walk'],run:['run','sprint'],jump:['jump'],fish:['fish'],farm:['farm','plant'],chop:['chop','axe'],attack:['attack'],dodge:['dodge','roll']};for(const [name,keys] of Object.entries(map)){const clip=chooseClip(clips,keys);if(clip)actions[name]=mixer.clipAction(clip,root)}let current=null;function play(name){const next=actions[name]||actions.idle||Object.values(actions)[0];if(!next||next===current)return;current?.fadeOut(.12);next.reset().fadeIn(.12).play();current=next}play('idle');return{root,mixer,actions,clips,play,update(dt){mixer.update(dt)}}}
globalThis.__AGCB_TEST_CHARACTER_RUNTIME={version:2,characters:TEST_CHARACTERS,create};
