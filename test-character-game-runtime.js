...
function load(url){return new Promise((resolve,reject)=>{let done=false;const timer=setTimeout(()=>{if(done)return;done=true;reject(new Error(`模型載入逾時：${url.split('/').pop()}`))},LOAD_TIMEOUT);loader.load(url,g=>{if(done)return;done=true;clearTimeout(timer);resolve(g)},undefined,e=>{if(done)return;done=true;clearTimeout(timer);reject(e)})})}
function normalize(root,targetHeight=2.0){root.updateMatrixWorld(true);const box=new THREE.Box3().setFromObject(root),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3());root.position.x-=center.x;root.position.z-=center.z;root.position.y-=box.min.y;root.scale.multiplyScalar(targetHeight/Math.max(size.y,.001));root.updateMatrixWorld(true);return root}
