// AG Cute Blocks V0.5.195 — formal CC0 adult game runtime adapter.
...
// Exact names discovered by V0.5.194 Chromium probe. Male and female share the same target skeleton naming.
const boneMap={
'root':'root','DEF-hips':'pelvis','DEF-spine001':'spine_01','DEF-spine002':'spine_02','DEF-spine003':'spine_03','DEF-neck':'neck_01','DEF-head':'Head',
'DEF-shoulderL':'clavicle_l','DEF-upper_armL':'upperarm_l','DEF-forearmL':'lowerarm_l','DEF-handL':'hand_l',
'DEF-f_index01L':'index_01_l','DEF-f_index02L':'index_02_l','DEF-f_index03L':'index_03_l','DEF-f_middle01L':'middle_01_l','DEF-f_middle02L':'middle_02_l','DEF-f_middle03L':'middle_03_l','DEF-f_pinky01L':'pinky_01_l','DEF-f_pinky02L':'pinky_02_l','DEF-f_pinky03L':'pinky_03_l','DEF-f_ring01L':'ring_01_l','DEF-f_ring02L':'ring_02_l','DEF-f_ring03L':'ring_03_l','DEF-thumb01L':'thumb_01_l','DEF-thumb02L':'thumb_02_l','DEF-thumb03L':'thumb_03_l',
'DEF-shoulderR':'clavicle_r','DEF-upper_armR':'upperarm_r','DEF-forearmR':'lowerarm_r','DEF-handR':'hand_r',
'DEF-f_index01R':'index_01_r','DEF-f_index02R':'index_02_r','DEF-f_index03R':'index_03_r','DEF-f_middle01R':'middle_01_r','DEF-f_middle02R':'middle_02_r','DEF-f_middle03R':'middle_03_r','DEF-f_pinky01R':'pinky_01_r','DEF-f_pinky02R':'pinky_02_r','DEF-f_pinky03R':'pinky_03_r','DEF-f_ring01R':'ring_01_r','DEF-f_ring02R':'ring_02_r','DEF-f_ring03R':'ring_03_r','DEF-thumb01R':'thumb_01_r','DEF-thumb02R':'thumb_02_r','DEF-thumb03R':'thumb_03_r',
'DEF-thighL':'thigh_l','DEF-shinL':'calf_l','DEF-footL':'foot_l','DEF-toeL':'ball_l','DEF-thighR':'thigh_r','DEF-shinR':'calf_r','DEF-footR':'foot_r','DEF-toeR':'ball_r'};
...
async function create(sex='male'){
 ...
 const root=targetG.scene.clone(true),source=srcG.scene.clone(true);root.name=`ag-formal-cc0-${variant}-v05195`;root.userData={agFormalCharacter:true,release:'V0.5.195',sex:variant};fitHeight(root,2);
 const sk=findSkeleton(root);if(!sk)throw new Error('V0.5.195 target skeleton missing');const src=sourceMap(source),dst=targetBones(sk),pairs=[];source.updateMatrixWorld(true);root.updateMatrixWorld(true);
 ...
 if(pairs.length<45)throw new Error(`V0.5.195 bone map insufficient ${pairs.length}`);
 const sourceHips=src['DEF-hips'],targetHips=dst['pelvis'];if(!sourceHips||!targetHips)throw new Error('V0.5.195 hips missing');const sourceHipsBind=sourceHips.position.clone(),targetHipsBind=targetHips.position.clone();
 ...
 for(const [k,names] of Object.entries(requested)){const c=chooseClip(clips,names);if(c)actions[k]=mixer.clipAction(c,source)}actions.fish=actions.interact||actions.idle;actions.sleep=actions.sit||actions.idle;for(const required of ['idle','walk','run','jump'])if(!actions[required])throw new Error(`V0.5.195 required action missing ${required}`);
 ...
 function update(dt){mixer.update(Math.max(0,Number(dt)||0));retarget()}play('idle');retarget();return{root,mixer,actions,clips,play,update,available:Object.keys(actions),animationLibraryStatus:`UAL1 Formal CC0 / mapped ${pairs.length}`,styleStatus:`V0.5.195 ${variant} TEST`,forwardYaw:0,get action(){return currentName},get mappedBones(){return pairs.length}};
}
globalThis.__AGCB_FORMAL_CC0_RUNTIME={version:195,release:'V0.5.195',status:'TEST_RUNTIME_ADAPTER',models:MODELS,animations:UAL1,createMale:()=>create('male'),createFemale:()=>create('female'),create};export{create};