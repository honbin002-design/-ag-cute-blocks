// AG Cute Blocks V0.5.191 — formal CC0 adult game runtime adapter.
...
function findSkeleton(root){let sk=null;root.traverse(o=>{if(!sk&&o.isSkeleton)sk=o});return sk}
...
async function create(sex='male'){
  ...
  const root=targetG.scene.clone(true),source=srcG.scene.clone(true);root.name=`ag-formal-cc0-${variant}-v05191`;root.userData={agFormalCharacter:true,release:'V0.5.191',sex:variant};fitHeight(root,2);
  const sk=findSkeleton(root);if(!sk)throw new Error('V0.5.191 target skeleton missing');const src=sourceMap(source),dst=targetBones(sk),pairs=[];
  ...
  if(pairs.length<20)throw new Error(`V0.5.191 bone map insufficient ${pairs.length}`);
  const sourceHips=src['DEF-hips'],targetHips=dst['pelvis'];if(!sourceHips||!targetHips)throw new Error('V0.5.191 hips missing');const sourceHipsBind=sourceHips.position.clone(),targetHipsBind=targetHips.position.clone();
  ...
  for(const required of ['idle','walk','run','jump'])if(!actions[required])throw new Error(`V0.5.191 required action missing ${required}`);
  ...
  function retarget(dt=0){source.updateMatrixWorld(true);root.updateMatrixWorld(true);for(const p of pairs){const cur=new THREE.Quaternion();p.s.getWorldQuaternion(cur);const delta=cur.multiply(p.sb.clone().invert()),desired=delta.multiply(p.db.clone()),parentQ=new THREE.Quaternion();if(p.d.parent)p.d.parent.getWorldQuaternion(parentQ);else parentQ.identity();p.d.quaternion.copy(parentQ.invert().multiply(desired));p.d.updateMatrixWorld(true)}const dy=sourceHips.position.y-sourceHipsBind.y;targetHips.position.copy(targetHipsBind);targetHips.position.y+=dy;root.updateMatrixWorld(true)}
  function update(dt){mixer.update(Math.max(0,Number(dt)||0));retarget(dt)}play('idle');retarget(0);
  return{root,mixer,actions,clips,play,update,available:Object.keys(actions),animationLibraryStatus:`UAL1 Formal CC0 / mapped ${pairs.length}`,styleStatus:`V0.5.191 ${variant} TEST`,forwardYaw:0,get action(){return currentName},get mappedBones(){return pairs.length}};
}
globalThis.__AGCB_FORMAL_CC0_RUNTIME={version:191,release:'V0.5.191',status:'TEST_RUNTIME_ADAPTER',models:MODELS,animations:UAL1,createMale:()=>create('male'),createFemale:()=>create('female'),create};
export{create};