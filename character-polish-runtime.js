...
function stateFor(g,host=g){let s=travelState.get(g);if(!s){s={x:host.position.x,z:host.position.z,phase:Math.random()*Math.PI*2,moving:false,motion:0};travelState.set(g,s)}return s}
...
function updateAvatarTravel(g){const u=g?.userData,p=u?.animatedParts,v=u?.visual;if(!u?.avatarStyle||!p||!v||['sit','lie','sleep','swing','dine'].includes(u.pose))return;const s=stateFor(g,g),dx=g.position.x-s.x,dz=g.position.z-s.z,travel=Math.hypot(dx,dz);s.x=g.position.x;s.z=g.position.z;s.moving=travel>.00055;s.motion=THREE.MathUtils.lerp(s.motion,s.moving?1:0,s.moving?.50:.24);if(s.moving)s.phase+=Math.min(.48,travel*20);const step=Math.sin(s.phase),contact=Math.abs(Math.sin(s.phase*2)),motion=s.motion;if(p.leftLeg)p.leftLeg.rotation.x=-step*.47*motion;if(p.rightLeg)p.rightLeg.rotation.x=step*.47*motion;if(p.leftArm)p.leftArm.rotation.x=step*.34*motion;if(p.rightArm)p.rightArm.rotation.x=-step*.34*motion;v.position.y=-contact*.018*motion;v.rotation.z=step*.010*motion}
function updatePetTravel(g){const u=g?.userData,p=u?.animatedParts,host=g.parent||g;if(!u?.petType||!Array.isArray(p?.legs))return;const s=stateFor(g,host);if(['sleep','petResponse'].includes(u.petState||'idle')){s.x=host.position.x;s.z=host.position.z;s.moving=false;s.motion=THREE.MathUtils.lerp(s.motion,0,.24);return}const dx=host.position.x-s.x,dz=host.position.z-s.z,travel=Math.hypot(dx,dz);s.x=host.position.x;s.z=host.position.z;s.moving=travel>.00045;s.motion=THREE.MathUtils.lerp(s.motion,s.moving?1:0,s.moving?.52:.24);if(s.moving)s.phase+=Math.min(.42,travel*18);const phase=s.phase,amp=u.petType==='cat'?.38:.35,lift=u.petType==='cat'?.030:.026,motion=s.motion;p.legs.forEach((leg,i)=>{const a=phase+(i===0||i===3?0:Math.PI),step=Math.sin(a);leg.rotation.z=0;leg.rotation.x=step*amp*motion;const base=leg.userData._agcbBaseY??(leg.userData._agcbBaseY=leg.position.y);leg.position.y=base+Math.max(0,step)*lift*motion});if(u.body){u.body.position.y=(u.baseBodyY||.48)-Math.abs(Math.sin(phase*2))*.012*motion;u.body.rotation.z=Math.sin(phase)*.012*motion}if(p.tail)p.tail.rotation.y=Math.sin(phase)*.28*motion}
