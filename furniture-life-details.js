...
style.textContent=`.furnitureExtra{position:fixed;z-index:86;left:50%;bottom:max(92px,calc(env(safe-area-inset-bottom) + 82px));transform:translateX(-50%);border:0;border-radius:15px;padding:10px 14px;background:#fff7cceb;color:#42565c;font-size:12px;font-weight:900;box-shadow:0 3px 12px #0003;display:none;pointer-events:auto}.furnitureExtra.show{display:block}`;
...
function showExtra(label,kind){currentKind=kind;extra.textContent=label;extra.classList.add('show')}
