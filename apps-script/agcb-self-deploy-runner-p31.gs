/**
 * AG Cute Blocks — Apps Script self-deploy runner P3.1
 * TEST only. Same Apps Script project/deployment.
 * CDN-pinned source transport + idempotent recovery.
...
function AGCB_SELF_DEPLOY_BOOTSTRAP_ONCE(){
  ...
  return Object.assign({runnerInstalled:true,pollMinutes:AGCB_SD_POLL_MINUTES,transport:'CDN_PINNED',runner:'P3.1'},r);
}
...
function agcbSdPreflight_(){
  ...
  const control=JSON.parse(agcbSdCdnFile_(AGCB_SD_CONTROL_PATH,'dev-v0.1','control-'+Date.now()));
...
function agcbSdDeployFromPrepared_(p,force){
  ...
  const lastRelease=String(props.getProperty('AGCB_DEPLOY_LAST_RELEASE')||'');
  if(!force&&lastRelease===c.releaseId){
  ...
  if(existingVersion){
    if(Number(p.previousVersion)!==Number(existingVersion)){
      agcbSdApi_('put',p.deploymentPath,{deploymentConfig:{
        scriptId:p.scriptId,
        versionNumber:Number(existingVersion),
        manifestFileName:'appsscript',
        description:'AG Cute Blocks TEST auto-recover '+c.releaseId
      }});
    }
    ...
    return {
      status:'RECOVERED_EXISTING_VERSION',releaseId:c.releaseId,previousVersion:p.previousVersion,versionNumber:Number(existingVersion),
      deploymentId:AGCB_SD_DEPLOYMENT_ID,newVersionCreated:false,transport:'CDN_PINNED',runner:'P3.1',
  ...
  return {
    ...
    deploymentId:AGCB_SD_DEPLOYMENT_ID,newVersionCreated:true,transport:'CDN_PINNED',runner:'P3.1',
    prodWriteEnabled:false,prodReadEnabled:false,restoreEnabled:false,deleteEnabled:false
  };
}

function agcbSdConfirmDeployment_(deploymentPath,targetVersion){
  for(let i=0;i<8;i++){
    if(i>0) Utilities.sleep(1200);
    const after=agcbSdApi_('get',deploymentPath);
    if(after&&after.deploymentId===AGCB_SD_DEPLOYMENT_ID&&Number(after.deploymentConfig&&after.deploymentConfig.versionNumber)===Number(targetVersion)) return after;
  }
  throw new Error('STOP: deployment 更新後多次反讀仍未指向 target version。');
}

function agcbSdWriteReleaseState_(props,releaseId,versionNumber){
  const now=new Date().toISOString();
  props.setProperties({
    AGCB_DEPLOY_LAST_RELEASE:String(releaseId),
...
function agcbSdListVersions_(scriptId){
  let rows=[];
  let pageToken='';
...
function agcbSdFindMatchingVersion_(scriptId,targetFiles){
  const rows=agcbSdListVersions_(scriptId).sort((a,b)=>Number(b.versionNumber||0)-Number(a.versionNumber||0));
  for(const row of rows){
    const n=Number(row.versionNumber||0);
    if(!n) continue;
    const content=agcbSdApi_('get','/projects/'+encodeURIComponent(scriptId)+'/content?versionNumber='+n);
    if(agcbSdFilesEqual_(content.files||[],targetFiles)) return n;
  }
  return 0;
}

function agcbSdFilesEqual_(actualFiles,targetFiles){
...
function agcbSdCdnFile_(path,ref,cacheKey){
  ...
  for(const url of urls){
    try{
      const r=UrlFetchApp.fetch(url,{method:'get',headers:{'Cache-Control':'no-cache','User-Agent':'AG-Cute-Blocks-Self-Deploy-P3.1'},muteHttpExceptions:true,followRedirects:true});
