/**
 * AG Cute Blocks — Apps Script self-deploy runner P2.3
 * TEST only. Same Apps Script project/deployment.
 * Important: after switching from the default GCP project to the standard
 * AG Cute Blocks TEST GCP project, the old /exec URL may temporarily return
 * an HTML 404. P2.3 therefore validates the immutable deployment and source
 * through the Apps Script API instead of depending on the public /exec probe.
 */
...
function agcbSdPreflight_(){
  ...
  // Verify the existing deployment only through the authenticated Apps Script API.
...
function agcbSdDeployFromPrepared_(p,force){
  ...
  // Post-deploy verification is also API based. This avoids false failure from
  // the public /exec URL while Google finishes propagating the new GCP binding.
  ...
  return {status:'DEPLOYED',releaseId:c.releaseId,previousVersion:p.previousVersion,versionNumber,deploymentId:AGCB_SD_DEPLOYMENT_ID,prodWriteEnabled:false,prodReadEnabled:false,restoreEnabled:false,deleteEnabled:false};
}
...
function agcbSdApi_(method,path,body){
  const opt={method,headers:{Authorization:'Bearer '+ScriptApp.getOAuthToken(),Accept:'application/json'},muteHttpExceptions:true};
  ...
  if(code<200||code>=300) throw new Error('STOP: Apps Script API '+method.toUpperCase()+' '+code+' '+text.slice(0,900));
