// AG Cute Blocks V0.5.103 — TEST-only Google Apps Script Drive backup endpoint.
// Adds one-time editor-only bootstrap helpers. PROD write/read, restore and delete remain disabled.
const AGCB_VERSION='0.5.103';
...
function doGet(){return agcbJson_({ok:true,project:AGCB_PROJECT,version:AGCB_VERSION,status:'TEST_BRIDGE_READY',prodWriteEnabled:false,prodReadEnabled:false,restoreEnabled:false,deleteEnabled:false})}
...
// Run this directly from the Apps Script editor once. It never runs through doGet/doPost.
// The returned token must be copied once into the TEST client setup UI/config and must not be committed to GitHub.
function agcbSetupTestBridge_(){
  const props=PropertiesService.getScriptProperties();
  let token=String(props.getProperty(AGCB_TOKEN_PROPERTY)||'');
  let created=false;
  if(token.length<32){token=(Utilities.getUuid()+Utilities.getUuid()).replace(/-/g,'');props.setProperty(AGCB_TOKEN_PROPERTY,token);created=true}
  const testFolder=DriveApp.getFolderById(AGCB_TEST_FOLDER_ID);
  if(testFolder.getName()!=='TEST')throw new Error('test-folder-id-name-mismatch');
  return{project:AGCB_PROJECT,version:AGCB_VERSION,environment:'TEST',created:created,token:token,tokenLength:token.length,testFolderId:AGCB_TEST_FOLDER_ID,testPath:AGCB_TEST_PATH,prodWriteEnabled:false,prodReadEnabled:false,restoreEnabled:false,deleteEnabled:false};
}
function agcbGetSetupStatus_(){const token=agcbToken_();return{project:AGCB_PROJECT,version:AGCB_VERSION,environment:'TEST',tokenConfigured:token.length>=32,testFolderId:AGCB_TEST_FOLDER_ID,testPath:AGCB_TEST_PATH,prodWriteEnabled:false,prodReadEnabled:false,restoreEnabled:false,deleteEnabled:false}}
