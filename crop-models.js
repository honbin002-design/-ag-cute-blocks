...
export function createCropModel(kind='carrot',stage=1){
  ...
  if(kind==='carrot'){
    ...
    if(stage>.68){const root=cone(g,.14,.42,0,.04,0,0xed7830,[Math.PI,0,0]);root.position.y=.03;ball(g,.105,0,.22,0,0xed7830,[1.05,.55,1.05],12)}
  }else if(kind==='corn'){
...
export function createFruitTreeModel(kind='apple',stage=1,season='spring'){
  const g=new THREE.Group();stage=Math.max(.18,Math.min(1,stage));
  const trunkH=.72+stage*1.55,trunkColor=kind==='peach'?0x8d6553:0x80614b;
  ...
  const palettes={apple:{leaf:0x5fa657,fall:0xcf8445},orange:{leaf:0x4d9851,fall:0x76a04d},peach:{leaf:0x70a95d,fall:0xd59255}};
  const pal=palettes[kind]||palettes.apple,leafColor=season==='autumn'?pal.fall:season==='winter'?(kind==='orange'?0x688e58:0xb6c2ac):pal.leaf;
  const crownY=trunkH*.90,crownScale=kind==='orange'?.90:kind==='peach'?1.02:.96;
  const lobes=stage<.42?[[0,0,0,.52]]:[[0,0,0,1],[.48,.02,.05,.72],[-.46,.06,.13,.74],[.12,.23,.38,.67],[-.10,.22,-.37,.68]];
  for(const [x,y,z,s] of lobes)ball(g,.67*stage*crownScale,x*stage,crownY+y*stage,z*stage,leafColor,[1,s,1],18);
  if(stage>.58&&season==='spring'){
    const blossom=kind==='peach'?0xf4a9b5:kind==='apple'?0xffe2e5:0xfff3df;
    for(let i=0;i<11;i++){const a=i*2.399,r=.30+.32*(i%3)/2;flower(g,Math.sin(a)*r,crownY-.08+(i%4)*.14,Math.cos(a)*r,blossom,.78)}
  }
  if(treeCanFruit(kind,season,stage)){
    const fc={apple:0xe45249,orange:0xee9332,peach:0xf09a82}[kind]||0xe45249;
    for(let i=0;i<10;i++){const a=i*2.399,r=.32+.34*(i%3)/2;const f=ball(g,.075+stage*.025,Math.sin(a)*r,crownY-.16+(i%4)*.14,Math.cos(a)*r,fc,kind==='peach'?[1.06,.92,1.03]:[1,1,1],13);stem(g,f.position.x,f.position.y+.06,f.position.z,.10,0x5f7e42,.012)}
  }
  ...
  g.userData={treeKind:kind,stage,season,fruitReady:treeCanFruit(kind,season,stage)};return g;
}
