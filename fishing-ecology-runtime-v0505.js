...
function isFishingCatchCallback(fn){if(typeof fn!=='function')return false;let src='';try{src=Function.prototype.toString.call(fn)}catch{return false}return src.includes('smallFish')&&src.includes('riverFish')&&src.includes('goldFish')&&src.includes('addInventory')}
function withEcology(delegate){if(injecting||!selectedFishingRod())return delegate();injecting=true;const ctx=context(),fish=chooseFish(ctx),roll=fishRollForLegacyThreshold(fish),originalSetTimeout=globalThis.setTimeout;let captured=false;globalThis.setTimeout=function(fn,delay,...args){const fishingDelay=Number(delay)>=850&&Number(delay)<=2200,fishingCallback=isFishingCatchCallback(fn);if(!captured&&fishingDelay&&fishingCallback){captured=true;return nativeSetTimeout(function(...cbArgs){const originalRandom=Math.random;let first=true;Math.random=function(){if(first){first=false;return roll}return originalRandom()};try{return fn(...cbArgs)}finally{Math.random=originalRandom}},delay,...args)}return nativeSetTimeout(fn,delay,...args)};try{return delegate()}finally{globalThis.setTimeout=originalSetTimeout;injecting=false}}
...
globalThis.__AGCB_FISHING_ECOLOGY={version:'0.5.05',schema:FISHING_ECOLOGY_SCHEMA,active:true,get context(){return context()},get weights(){return getFishingWeights(context())},get timeBand(){return fishingTimeBand(context().minute)},callbackFilter:'fish-inventory-signature'};
