...
function createPetBase(type,colors){const g=new THREE.Group(),bodyMat=mat(colors.body,.94),detailMat=mat(colors.detail,.94),muzzleMat=mat(colors.muzzle,.94),noseMat=mat(0x2b2c2d,.72),dog=type==='dog';const body=ellipsoid(g,dog?.36:.32,0,.48,.08,bodyMat,dog?[1.55,.78,.88]:[1.48,.76,.84],24);ellipsoid(g,dog?.23:.215,0,.62,-.49,bodyMat,dog?[1.05,1,.94]:[1,.98,.9],22);ellipsoid(g,.19,0,.50,-.28,bodyMat,[.95,1.22,.9],18);const legs=[];for(const [x,z] of [[-.22,-.12],[.22,-.12],[-.22,.30],[.22,.30]]){const p=capsule(g,dog?.055:.05,.34,x,.23,z,bodyMat);legs.push(p);paw(g,x,z-.035,detailMat)}ellipsoid(g,dog?.105:.085,0,.57,-.69,muzzleMat,dog?[1.25,.72,1.12]:[1.15,.65,1.05],16);ellipsoid(g,.047,0,.60,-.78,noseMat,[1.08,.72,.8],12);for(const x of[-.095,.095]){ellipsoid(g,.031,x,.69,-.67,eye,[1,.98,.58],10);ellipsoid(g,.009,x+.009,.70,-.686,white,[1,1,.4],8)}let tail;if(dog){const le=ellipsoid(g,.14,-.20,.72,-.43,detailMat,[.62,1.25,.48],16),re=ellipsoid(g,.14,.20,.72,-.43,detailMat,[.62,1.25,.48],16);le.rotation.z=-.25;re.rotation.z=.25;tail=capsule(g,.045,.46,0,.55,.55,bodyMat);tail.rotation.x=-.78;tail.position.z=.52}else{cone(g,.105,.25,-.15,.88,-.47,bodyMat,[0,0,-.08]);cone(g,.105,.25,.15,.88,-.47,bodyMat,[0,0,.08]);tail=new THREE.Group();tail.position.set(0,.50,.54);const t1=capsule(tail,.038,.48,0,.12,.12,bodyMat);t1.rotation.x=-.88;const t2=capsule(tail,.034,.42,0,.18,.36,bodyMat);t2.rotation.x=-.38;g.add(tail);for(let s=-1;s<=1;s+=2)for(let i=0;i<3;i++){const w=new THREE.Mesh(new THREE.CylinderGeometry(.0045,.0045,.28,5),detailMat);w.rotation.z=Math.PI/2;w.position.set(s*.13,.56,-.72);w.rotation.y=(i-1)*.12;g.add(w)}}g.userData={petType:type,animatedParts:{tail,legs},body};return g}
...
export function setCuteCharacterPose(group,pose='idle'){
  ...
  p.leftArm.rotation.x=0;p.rightArm.rotation.x=0;p.leftLeg.rotation.x=0;p.rightLeg.rotation.x=0;
  if(pose==='sit'){p.leftLeg.rotation.x=-1.22;p.rightLeg.rotation.x=-1.22;p.leftArm.rotation.x=-.18;p.rightArm.rotation.x=-.18}
  else if(pose==='lie'){if(v){v.rotation.z=Math.PI/2;v.position.set(.66,0,0)}p.leftArm.rotation.x=.08;p.rightArm.rotation.x=-.08;p.leftLeg.rotation.x=.05;p.rightLeg.rotation.x=-.05}
}
export function animateCuteCharacter(group,time,moving=false,speed=1){const u=group?.userData,p=u?.animatedParts;if(!p)return;if(u.pose==='sit'||u.pose==='lie')return;const rate=.011*Math.max(.65,speed),swing=moving?Math.sin(time*rate)*.46:Math.sin(time*.003)*.018;p.leftArm.rotation.x=swing;p.rightArm.rotation.x=-swing;p.leftLeg.rotation.x=-swing*.78;p.rightLeg.rotation.x=swing*.78;if(Array.isArray(p.legs))p.legs.forEach((leg,i)=>leg.rotation.x=moving?Math.sin(time*rate+(i===0||i===3?0:Math.PI))*.34:0);if(p.tail)p.tail.rotation.y=Math.sin(time*(moving?.011:.0055))*.30}
