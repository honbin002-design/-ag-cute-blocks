...
export function createCuteChildAvatar(style='girl',options={}){
  ...
  ellipsoid(g,.16,-.23,1.91,-.29,hair,[1.10,.78,.46],18);ellipsoid(g,.18,-.06,1.96,-.31,hair,[1.12,.82,.44],18);ellipsoid(g,.16,.12,1.94,-.30,hair,[1.10,.78,.46],18);ellipsoid(g,.13,.26,1.88,-.27,hair,[1.04,.78,.44],16);
  ...
  ellipsoid(g,.052,-.135,1.76,-.345,eye,[1,.98,.54],14);ellipsoid(g,.052,.135,1.76,-.345,eye,[1,.98,.54],14);ellipsoid(g,.016,-.119,1.778,-.375,white,[1,1,.4],10);ellipsoid(g,.016,.151,1.778,-.375,white,[1,1,.4],10);
  ...
  addAvatarAccessories(g,c,hair,shirt);const visual=new THREE.Group();while(g.children.length)visual.add(g.children[0]);g.add(visual);const bodyScale={round:[1.05,1,1.02],tall:[.95,1.07,.97],petite:[.94,.94,.95]}[c.body]||[1,1,1];g.scale.set(...bodyScale);g.userData={avatarStyle:c.gender,avatarCustomization:c,avatarVisualStyle:'premium-chibi-v3',visual,pose:'idle',animatedParts:{leftArm,rightArm,leftLeg,rightLeg,bib}};
...
function addAvatarAccessories(g,c,hair,shirt){
  ...
  // premium-chibi-v3: base proportions and details are intentionally readable on a phone.
  for(const x of[-.135,.135])ellipsoid(g,.108,x,1.76,-.315,white,[1,.94,.40],18);
  for(const x of[-.135,.135]){ellipsoid(g,.058,x,1.76,-.382,eye,[1,.98,.52],14);ellipsoid(g,.022,x-.018,1.785,-.412,white,[1,1,.45],10)}
  for(const [x,y,r,sx] of[[-.34,2.02,-.56,.82],[-.23,2.10,-.30,.98],[-.08,2.15,-.10,1.02],[.09,2.14,.12,1.00],[.23,2.08,.34,.90],[.34,2.00,.52,.76]]){const lock=ellipsoid(g,.135,x,y,-.265,hair,[sx,.80,.48],18);lock.rotation.z=r}
  ...
  if(c.glasses==='round'){for(const x of[-.135,.135]){const lens=new THREE.Mesh(new THREE.TorusGeometry(.085,.012,8,20),frame);lens.position.set(x,1.76,-.415);lens.castShadow=true;g.add(lens)}const bridge=new THREE.Mesh(new THREE.BoxGeometry(.09,.014,.014),frame);bridge.position.set(0,1.76,-.415);g.add(bridge)}
  ...
  if(c.accessory==='scarf'){ellipsoid(g,.38,0,1.39,0,accent,[.75,.20,.60],18);const tail=ellipsoid(g,.09,.23,1.17,-.04,accent,[.68,1.9,.68],14);tail.rotation.z=-.12}else if(c.accessory==='backpack'){const pack=new THREE.Mesh(new THREE.BoxGeometry(.32,.40,.16),mat(0xf09c6d,.9));pack.position.set(0,1.04,.36);pack.castShadow=true;g.add(pack)}else if(c.accessory==='bow'){ellipsoid(g,.12,-.14,1.57,-.40,accent,[1.30,.84,.34],14);ellipsoid(g,.12,.14,1.57,-.40,accent,[1.30,.84,.34],14);ellipsoid(g,.04,0,1.57,-.43,hatMat,[1,1,.6],10)}
}
