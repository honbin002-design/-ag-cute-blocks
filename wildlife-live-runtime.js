...
const models=new Map(),perf=createPerformanceGovernor();
...
function spawn(){
  ...
  globalThis.__AGCB_WILDLIFE_RUNTIME={population,models,performance:perf};ready=true;last=performance.now();requestAnimationFrame(loop);
}
...
function loop(now){
  ...
  for(const rec of models.values()){
    ...
    if(creatureUpdateAllowed(frame,distance,perf.tier))updateWildlifeEntity(entity,{playerX:p.x,playerZ:p.z,timeSeconds:now/1000,dt,quality:perf.tier});
    ...
    const shadow=shouldCastCreatureShadow(distance,perf.tier);if(rec.shadow!==shadow){rec.shadow=shadow;model.traverse(m=>{if(m.isMesh)m.castShadow=shadow})}
  }
}
init();
