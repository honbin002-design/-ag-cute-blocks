...
export function createCow(){
  ...
  const body=ell(g,.43,0,.73,.10,hide,[1.62,.88,.90],24),neck=ell(g,.29,0,.77,-.42,hide,[.90,1.20,.82],20),headPivot=new THREE.Group();headPivot.position.set(0,.83,-.70);g.add(headPivot);
  ell(headPivot,.28,0,0,0,hide,[1.02,.86,.94],22);ell(headPivot,.16,0,-.11,-.21,pink,[1.35,.66,.90],16);ell(g,.18,-.31,.83,.02,dark,[1.2,.65,.28],14);ell(g,.16,.34,.67,.23,dark,[1.1,.72,.25],14);ell(headPivot,.12,.17,.10,.04,dark,[.9,.65,.25],12);
  const legs=[leg(g,-.28,-.16,hide,.58),leg(g,.28,-.16,hide,.58),leg(g,-.30,.36,hide,.58),leg(g,.30,.36,hide,.58)];
  for(const x of[-.22,.22]){const ear=ell(headPivot,.105,x,.10,.03,dark,[1.35,.48,.70],14);ear.rotation.z=x<0?-.20:.20;cone(headPivot,.042,.20,x*.72,.25,.08,0xe1d0a4,[0,0,x<0?.58:-.58])}for(const x of[-.105,.105])eye(headPivot,x,.04,-.24);
...
export function createSheep(){
  const g=new THREE.Group(),wool=M(0xf0eee7),face=M(0x625b56),hoof=0x493f3b,body=ell(g,.39,0,.62,.08,wool,[1.52,.94,.92],22);
  ...
  const legs=[leg(g,-.23,-.12,face,.48,hoof),leg(g,.23,-.12,face,.48,hoof),leg(g,-.24,.30,face,.48,hoof),leg(g,.24,.30,face,.48,hoof)];for(const x of[-.18,.18]){const ear=ell(headPivot,.085,x,.11,.01,face,[1.45,.45,.70],12);ear.rotation.z=x<0?-.25:.25}for(const x of[-.085,.085])eye(headPivot,x,.04,-.18);
...
export function createChicken(){
  const g=new THREE.Group(),feather=M(0xe8d2ad),wing=M(0xd2b588),red=M(0xc84f49),legMat=M(0xc9913c),body=ell(g,.28,0,.39,.04,feather,[1.02,1.05,1.34],20),headPivot=new THREE.Group();headPivot.position.set(0,.65,-.23);g.add(headPivot);ell(headPivot,.17,0,0,0,feather,[1,.98,.92],18);
  const wl=ell(g,.17,-.23,.42,.05,wing,[.48,1,1.18],16),wr=ell(g,.17,.23,.42,.05,wing,[.48,1,1.18],16);wl.rotation.z=.15;wr.rotation.z=-.15;cone(headPivot,.07,.20,0,-.02,-.22,0xd99837,[Math.PI/2,0,0]);for(const x of[-.072,.072])eye(headPivot,x,.04,-.16);for(let i=0;i<3;i++)ell(headPivot,.05,(i-1)*.055,.18,.05,red,[.75,1.15,.72],10);ell(headPivot,.055,0,-.09,-.14,red,[.75,1.1,.7],10);
  const legs=[];for(const x of[-.09,.09]){const p=new THREE.Group();p.position.set(x,.20,.06);const shank=new THREE.Mesh(new THREE.CylinderGeometry(.018,.021,.30,7),legMat);p.add(shank);g.add(p);legs.push(p);for(const sx of[-.035,0,.035]){const toe=new THREE.Mesh(new THREE.CylinderGeometry(.008,.008,.17,6),legMat);toe.rotation.x=Math.PI/2;toe.position.set(sx,-.15,-.08);p.add(toe)}}for(const x of[-.09,0,.09]){const f=ell(g,.11,x,.48,.34,wing,[.45,1.25,.62],12);f.rotation.x=-.45;f.rotation.z=x*.8}
  g.userData={animalType:'chicken',legs,wings:[wl,wr],headPivot,body,phase:Math.random()*6.28,...baseState('chicken',.65,.39)};return g;
}
export function setAnimalVisualState(g,state='idle',now=performance.now()){const u=g?.userData;if(!u?.legs)return;u.state=state;u.stateChangedAt=now}
export function animateAnimal(g,time,speed=0,state='auto'){
  const u=g.userData||{},legs=u.legs;if(!legs)return;const type=u.animalType;if(state!=='auto'&&state!==u.state)setAnimalVisualState(g,state,time);const s=state==='auto'?(speed>.001?'walk':(u.state||'idle')):state;
