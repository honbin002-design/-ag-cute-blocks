...
function leg(g,x,z,c,h=.55,hoof=0x4d423c){const p=new THREE.Group();p.position.set(x,h/2,z);p.userData.baseY=p.position.y;const m=new THREE.Mesh(new THREE.CapsuleGeometry(.06,Math.max(.04,h-.12),5,9),typeof c==='number'?M(c):c);m.castShadow=true;p.add(m);g.add(p);ell(g,.078,x,.07,z-.025,hoof,[1,.52,1.32],12);return p}
...
function baseState(type,headY,bodyY){return {state:'idle',baseHeadY:headY,baseBodyY:bodyY,stateChangedAt:0,holdPosition:null,gaitPhase:Math.random()*Math.PI*2,_motionAt:0,locomotionSchema:2}}
...
export function createChicken(){
  ...
  const legs=[];for(const x of[-.09,.09]){const p=new THREE.Group();p.position.set(x,.20,.06);p.userData.baseY=p.position.y;const shank=new THREE.Mesh(new THREE.CylinderGeometry(.018,.021,.30,7),legMat);p.add(shank);g.add(p);legs.push(p);for(const [sx,ang,len] of [[-.035,.08,.18],[0,0,.20],[.035,-.08,.18]]){const toe=new THREE.Mesh(new THREE.CylinderGeometry(.008,.008,len,6),legMat);toe.rotation.x=Math.PI/2;toe.rotation.z=ang;toe.position.set(sx,-.15,-.09);p.add(toe)}const backToe=new THREE.Mesh(new THREE.CylinderGeometry(.007,.007,.10,6),legMat);backToe.rotation.x=Math.PI/2;backToe.position.set(0,-.15,.05);p.add(backToe)}for(const x of[-.09,0,.09]){const f=ell(g,.11,x,.48,.35,wing,[.45,1.25,.62],12);f.rotation.x=-.45;f.rotation.z=x*.8}
...
function advanceGait(u,time,speed,type,moving){const dt=u._motionAt?Math.min(80,Math.max(0,time-u._motionAt)):16.67;u._motionAt=time;if(!moving)return u.gaitPhase||0;const travel=Math.max(0,speed)*(dt/16.67),stride=type==='chicken'?22:type==='sheep'?17:15;u.gaitPhase=(u.gaitPhase||0)+travel*stride;return u.gaitPhase}
function resetLegs(legs){legs.forEach(p=>{p.rotation.z=0;p.rotation.x=0;if(p.userData?.baseY!==undefined)p.position.y=p.userData.baseY})}
export function animateAnimal(g,time,speed=0,state='auto'){
  const u=g.userData||{},legs=u.legs;if(!legs)return;const type=u.animalType;if(state!=='auto'&&state!==u.state)setAnimalVisualState(g,state,time);const forced=['sleep','eat','drink','petResponse'].includes(u.state),s=state==='auto'?(forced?u.state:(speed>.001?'walk':'idle')):state,moving=s==='walk'&&speed>.001,gait=advanceGait(u,time,speed,type,moving);
  if(forced&&u.holdPosition&&g.parent)g.parent.position.copy(u.holdPosition);
  if(!animalAnimationDue(u,time,moving,forced))return;
  const head=u.headPivot,body=u.body,baseHeadY=u.baseHeadY??(type==='cow'?.84:type==='sheep'?.73:.66),baseBodyY=u.baseBodyY??(type==='cow'?.73:type==='sheep'?.62:.39);if(head){head.position.y=baseHeadY;head.rotation.set(0,0,0)}if(body){body.position.y=baseBodyY;body.rotation.set(0,0,0)}if(u.wings){u.wings[0].rotation.z=.15;u.wings[1].rotation.z=-.15}resetLegs(legs);g.position.y=0;
  if(s==='walk'){
    const amp=type==='chicken'?.50:type==='sheep'?.39:.35,lift=type==='chicken'?.055:type==='sheep'?.030:.026;legs.forEach((p,i)=>{const diagonal=type==='chicken'?i%2:(i===0||i===3?0:1),a=gait+diagonal*Math.PI,step=Math.sin(a);p.rotation.x=step*amp;if(p.userData?.baseY!==undefined)p.position.y=p.userData.baseY+Math.max(0,step)*lift});
    const stride=Math.sin(gait),contact=Math.abs(Math.sin(gait*2));if(head){head.rotation.x=(type==='chicken'?-.10:-.045)*stride;head.position.y=baseHeadY+(type==='chicken'?.018:.010)*contact}if(body){body.rotation.z=stride*(type==='chicken'?.024:.016);body.rotation.x=Math.sin(gait*2)*(type==='chicken'?.014:.008);body.position.y=baseBodyY-contact*(type==='chicken'?.014:.009)}g.position.y=contact*(type==='chicken'?.012:.007)
  }
  else if(s==='eat'||s==='drink'){const dip=s==='drink'?1:.82,peck=type==='chicken'?Math.max(0,Math.sin(time*.010+(u.phase||0))):.92+.08*Math.sin(time*.003+(u.phase||0));if(head){head.position.y=baseHeadY-(type==='cow'?.22:type==='sheep'?.18:.11)*dip*peck;head.rotation.x=(type==='chicken'?.78:.72)*dip*peck}if(type==='chicken'&&u.wings){u.wings[0].rotation.z=.10;u.wings[1].rotation.z=-.10}}
  else if(s==='sleep'){if(body){body.position.y=baseBodyY-(type==='chicken'?.16:.22);body.rotation.z=type==='chicken'?.06:.03}legs.forEach((p,i)=>{p.rotation.x=type==='chicken'?(i?-.72:.72):(i<2?-.86:.82);p.rotation.z=i%2?-.10:.10});if(head){head.position.y=baseHeadY-(type==='chicken'?.23:.30);head.rotation.x=type==='chicken'?.40:.30;head.rotation.z=.08*Math.sin(u.phase||0)}if(u.wings){u.wings[0].rotation.z=.03;u.wings[1].rotation.z=-.03}g.position.y=Math.sin(time*.0015+(u.phase||0))*.004}
  else if(s==='petResponse'){if(head){head.rotation.z=Math.sin(time*.006+(u.phase||0))*.10;head.rotation.x=-.04}if(body)body.rotation.z=Math.sin(time*.005+(u.phase||0))*.014}
  else{const idleWave=Math.sin(time*.0015+(u.phase||0));if(head){if(type==='chicken'){const peck=Math.max(0,Math.sin(time*.004+(u.phase||0))-.72)/.28;head.rotation.x=peck*.72;head.position.y=baseHeadY-peck*.08}else{const graze=Math.max(0,Math.sin(time*.0018+(u.phase||0))-.45)/.55;head.rotation.x=graze*.72;head.position.y=baseHeadY-graze*(type==='cow'?.18:.14)}}if(body)body.rotation.z=idleWave*.006}
  if(u.tail)u.tail.rotation.z=Math.sin((s==='walk'?gait:time*.0035)+(u.phase||0))*(s==='petResponse'?.22:s==='walk'?.16:.10);if(u.wings&&s!=='sleep'&&s!=='eat'&&s!=='drink'){const w=s==='walk'?Math.sin(gait)*.050:Math.sin(time*.003+(u.phase||0))*.028;u.wings[0].rotation.z=.15+w;u.wings[1].rotation.z=-.15-w}
}
