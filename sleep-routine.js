...
style.textContent=`
 .sleepMorning{position:fixed;z-index:88;left:auto;right:max(18px,calc(env(safe-area-inset-right) + 12px));top:38%;transform:translateY(-50%);border:0;border-radius:18px;padding:12px 17px;background:#304766e8;color:white;font-size:14px;font-weight:900;box-shadow:0 5px 22px #0004;display:none;pointer-events:auto;touch-action:manipulation}.sleepMorning.show{display:block}.sleepFade{position:fixed;z-index:180;inset:0;background:#263858;opacity:0;pointer-events:none;transition:opacity .42s ease;display:flex;align-items:center;justify-content:center;color:white;font-size:22px;font-weight:900}.sleepFade.on{opacity:1}.sleepFade small{display:block;font-size:12px;text-align:center;margin-top:8px;opacity:.75}@media(max-height:430px){.sleepMorning{top:34%;right:max(14px,calc(env(safe-area-inset-right) + 10px));padding:10px 14px;font-size:12px}}`;
