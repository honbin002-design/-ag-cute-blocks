...
// Third-person camera zoom is layered as a separate guarded feature patch so the
// original three recovery signatures and their CI contract stay unchanged.
const cameraPatches=[
  {
  ...
  },
  {
    ...
    to:'farmYaw=Number.isFinite(Number(settings.farmYaw))?Number(settings.farmYaw):FARM_YAW;let thirdDistance=Math.max(THIRD_CAMERA_DISTANCE_MIN,Math.min(8,Number(settings.thirdDistance)||THIRD_CAMERA_DISTANCE_DEFAULT));\nlet playerColor='
  },
  {
    id:'third-camera-distance-save',
    from:'cameraMode,farmDistance,farmPitch,farmYaw,season',
    to:'cameraMode,thirdDistance,farmDistance,farmPitch,farmYaw,season'
  },
  {
  ...
  {
    id:'third-camera-pinch-look-guard',
    from:'if(e.pointerId!==lookId)return;yaw-=',
    to:"if(cameraMode==='third'&&thirdPinchDistance>0)return;if(e.pointerId!==lookId)return;yaw-="
  },
  {
  ...
  {
    id:'third-camera-render-distance',
    from:'eye.x+Math.sin(yaw)*THIRD_CAMERA_DISTANCE,eye.y+1.85,eye.z+Math.cos(yaw)*THIRD_CAMERA_DISTANCE',
    to:'eye.x+Math.sin(yaw)*thirdDistance,eye.y+1.85,eye.z+Math.cos(yaw)*thirdDistance'
  }
];
const cameraApplied=[];
